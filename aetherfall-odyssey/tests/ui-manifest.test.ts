...
const assetRoot = resolve(process.cwd(), 'assets/ui');
...
describe('game-default UI asset manifest', () => {
  it('keeps one GUID per self-contained UI pack', async () => {
    ...
    const exploration = await readPack('aetherfall-hud.pack.json');
    ...
    const explorationAsset = (exploration.assets as Array<{ guid: string; kind: string }>)[0];
    const hudGuid = hudAsset?.guid;
    const settingsGuid = settingsAsset?.guid;
    const explorationGuid = explorationAsset?.guid;
    expect(hudAsset?.kind).toBe('ui');
    expect(settingsAsset?.kind).toBe('ui');
    expect(explorationAsset?.kind).toBe('ui');
    expect(hudGuid).toMatch(/^[0-9a-f-]{36}$/);
    expect(settingsGuid).toMatch(/^[0-9a-f-]{36}$/);
    expect(explorationGuid).toMatch(/^[0-9a-f-]{36}$/);
    expect(hudGuid).not.toBe(settingsGuid);
    expect(explorationGuid).not.toBe(hudGuid);
    expect(explorationGuid).not.toBe(settingsGuid);
  });
  ...
  it('stores the final HTML and CSS payload without an importer or DDC', async () => {
    ...
    const hudHtml = (hud.assets as Array<{ payload: { html: string } }>)[0]?.payload.html;
