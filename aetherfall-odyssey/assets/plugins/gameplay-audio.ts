...
const setSource = (world: World, player: EntityHandle, clip: ClipHandle, playing: boolean): boolean => {
...
export async function installGameplayAudio(
  ...
  if (!assets) {
    ...
    if (!parsed.ok) {
      ...
      if (loaded.ok) {
        const callerGrant = world.allocSharedRef('AudioClipAsset', loaded.value);
        const assigned = setSource(world, player, callerGrant, false);
        // GameplayAudio keeps the alloc-grant while AudioSource owns its
        // retained reference. World.set releases the old shared scalar before
        // retaining the new value, so same-handle rewrites need this external
        // owner to keep the slot generation alive between those two steps.
        if (assigned) clip = callerGrant;
        else world.sharedRefs.release(callerGrant).unwrap();
      } else {
  ...
  if (assets) {
    ...
    if (!parsed.ok) {
      ...
      if (loaded.ok) {
        const callerGrant = world.allocSharedRef('AudioClipAsset', loaded.value);
        const assigned = world.set(musicEntity, AudioSource, { clip: callerGrant, playing: false, loop: true, volume: 1, spatialBlend: 0, bus: 'music' });
        if (assigned.ok) musicClip = callerGrant;
        else {
          world.sharedRefs.release(callerGrant).unwrap();
          console.warn('[game] background music AudioSource update failed:', assigned.error.code, assigned.error.hint);
        }
      } else {
  ...
  return {
    dispose() {
      ...
      const ownedMusicClip = musicClip;
      musicPlaying = false;
      if (world.get(musicEntity, AudioSource).ok) world.despawn(musicEntity).unwrap();
      if (world.get(player, AudioSource).ok) world.removeComponent(player, AudioSource).unwrap();
      clip = HANDLE_NONE;
      musicClip = HANDLE_NONE;
      armed = false;
      // Drop component holders first, then the independent GameplayAudio
      // grants. This makes Stop safe whether the entities still exist or were
      // already removed by another lifecycle owner.
      if (ownedMusicClip !== HANDLE_NONE) world.sharedRefs.release(ownedMusicClip).unwrap();
      if (ownedClip !== HANDLE_NONE) world.sharedRefs.release(ownedClip).unwrap();
    },
    rearm() {
      if (!armed) return;
