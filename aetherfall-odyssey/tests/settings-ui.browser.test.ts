...
describe('game-default settings modal', () => {
  it('supports open, escape, memory updates and focus restore', () => {
    ...
    root.append(sibling);
    let projectedHighContrast = false;
    const handle = mountSettings(
      { guid: SETTINGS_UI_GUID, html: '<section role="dialog" hidden><div role="document" tabindex="-1"><input data-ui-setting="music"><input data-ui-setting="music-muted" type="checkbox"><input data-ui-setting="high-contrast" type="checkbox"><button data-ui-action="close-settings">Done</button></div></section>', css: '' },
      root,
      createGameSettingsState(),
      trigger,
      undefined,
      (state) => { projectedHighContrast = state.highContrast; },
    );
    ...
    const highContrast = handle.instance.host.shadowRoot?.querySelector<HTMLInputElement>('[data-ui-setting="high-contrast"]');
    if (highContrast) { highContrast.checked = true; highContrast.dispatchEvent(new Event('input', { bubbles: true })); }
    expect(handle.state.highContrast).toBe(true);
    expect(projectedHighContrast).toBe(true);
