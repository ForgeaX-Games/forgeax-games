...
export const SCENE_GUID = '76a15ebd-0d8c-4a0e-ad3f-e27d553f667d';
...
export async function loadScene(ctx: GameContext): Promise<LoadedScene | null> {
  ...
  const instance = instantiateSceneAsset(ctx.world, ctx.assets, loaded.value);
  if (!instance.ok) return null;
  let result: LoadedScene | null = null;
  try {
    ...
    if (result === null) ctx.world.despawnScene(instance.value);
  }
}
...
export function attachScenePhysics(ctx: GameContext, loaded: LoadedScene): ScenePhysics {
  ...
  for (const node of loaded.nodes) {
    ...
    // Collider primitives are authored in local space; Transform scale is
    // applied by physics. Feeding world-scaled extents here squares the visual
    // scale and can create invisible walls many metres beyond a landmark.
    const box = (restitution: number) => world.addComponent(entity, { component: Collider, data: { shape: ColliderShapeValue.cuboid, halfExtents: [0.5, 0.5, 0.5], restitution, friction: 0.7 } });
    const sphere = (restitution: number) => world.addComponent(entity, { component: Collider, data: { shape: ColliderShapeValue.sphere, radius: 0.5, restitution, friction: 0.6 } });
    ...
    switch (name) {
      ...
      default:
        if (name.startsWith('Crate')) {
          ...
          box(0.1);
          props.push({ e: entity, materials: materialsOf(entity) });
          world.addComponent(entity, { component: ScoringTarget, data: { points: 5 } });
        } else if (
          name.endsWith('Trunk') ||
          name.startsWith('Sanctuary') ||
          name === 'LastLightBeacon' ||
          name === 'BeaconObservatoryIsland' ||
          name === 'AncientObservatoryDome' ||
          name.startsWith('ObservatoryColumn') ||
          name.startsWith('StormBridge') ||
          name.startsWith('MemoryShrine_') ||
          name.includes('Arch') ||
          name.includes('Spire') ||
