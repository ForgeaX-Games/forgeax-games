...
export interface CustomProjectileMeshStore {
  ...
  evictTexture?(handle: Handle<"TextureAsset", "shared">): unknown;
  evictMesh?(handle: Handle<"MeshAsset", "shared">): unknown;
}

export interface CustomProjectileMesh {
  readonly textureHandle: Handle<"TextureAsset", "shared">;
...
export async function createCustomProjectileMesh(
  ...
  const dispose = (): void => {
    ...
    for (let index = cleanupSteps.length - 1; index >= 0; index -= 1) {
      try {
        ...
        console.error(
          "[aetherfall-projectile] custom mesh cleanup failed",
          cleanupError,
        );
      }
    }
  ...
  cleanupSteps.push(() => world.sharedRefs.release(textureHandle).unwrap());
  const store = renderer.store as CustomProjectileMeshStore;
  try {
    ...
    cleanupSteps.push(() => store.evictTexture?.(textureHandle));
    ...
    cleanupSteps.push(() => world.sharedRefs.release(meshHandle).unwrap());
    ...
    cleanupSteps.push(() => store.evictMesh?.(meshHandle));
    ...
    cleanupSteps.push(() => world.sharedRefs.release(materialHandle).unwrap());
    const spriteMaterial = world.allocSharedRef<"MaterialAsset", MaterialAsset>(
      "MaterialAsset",
      {
        kind: "material",
        passes: [
    ...
    cleanupSteps.push(() => world.sharedRefs.release(spriteMaterial).unwrap());
    const spriteLitMaterial = world.allocSharedRef<
      "MaterialAsset",
      MaterialAsset
    >("MaterialAsset", {
      kind: "material",
    ...
    cleanupSteps.push(() =>
      world.sharedRefs.release(spriteLitMaterial).unwrap(),
    );
    return {
      textureHandle,
      meshHandle,
...
export function toggleCustomProjectileMesh(mesh: CustomProjectileMesh): void {
  ...
  if (
    mesh.store.getMeshGpuHandles !== undefined &&
    mesh.store.getMeshGpuHandles(mesh.meshHandle) === undefined
  )
    return;
  ...
  mesh.store.updateMesh(
    mesh.meshHandle,
    mesh.uvMode === "upper" ? mesh.baseVertices : mesh.alternateVertices,
    INDICES,
  );
}
